...
//-----------【EDIT】
const uint8_t SOL_ASCII_IME_lowerchar[]	="abcdefghijklmnopqrstuvwxyz";
const uint8_t SOL_ASCII_IME_upperchar[]	="ABCDEFGHIJKLMNOPQRSTUVWXYZ";
const uint8_t SOL_ASCII_IME_Number[]		="0123456789+-*/=%()[]{}<>&|~^";
const uint8_t SOL_ASCII_IME_symbol[]		=" .,:;_?!@#$'\"\\";
const uint8_t SOL_ASCII_IME_CRLchar[]	={0x00,0x0a,0x0d};	//控制类字符
...
//-----------【选项使能表】
uint8_t option_enable_list[OPTIONS_MAX];	//选项使能表,此处可以改进

//-----------【GotoPage控件】
extern MENU_PAGE *current_page;		//当前页面

//-----------【占用标志寄存器】
extern uint8_t SOLGUI_CSR;		//设置模式控制寄存器

//################################【内部调用函数】##############################

boolean _OptionsDisplay_Judge(uint8_t USN)
{
...
uint32_t _OptionsDisplay_coorY(uint8_t USN)
{
...
void SOLGUI_Cursor(uint8_t rowBorder_Top,uint8_t rowBorder_Bottom,uint8_t option_num)		//光标（上边界行，下边界行，选项个数）	
{
	...
	uint8_t cursor_abs_offset=0;	//光标的绝对偏移

	double s_h=0;			//滑块的高度
	uint32_t s_y=0;				//滑块的位置
//--------【参数过滤】
...
void SOLGUI_Widget_GotoPage(uint8_t USN,MENU_PAGE *page)		//页面跳转控件
{
	uint32_t y_disp=0;	//该选项应该显示的行位置
	uint8_t cur_key=0;	//键值
...
double _Pow_10(int8_t n) 	//pow=10^n
{
	int16_t i=0;
	int8_t m=(n>=0)?n:-n;
	double p=1;
...
void SOLGUI_Widget_Spin(uint8_t USN,const uint8_t *name,uint8_t type,double max,double min,void* value)		//数字旋钮控件
{
	double swap;	
	uint32_t y_disp=0;	//该选项应该显示的行位置
	uint8_t cur_key=0;	//键值

	static int8_t spin_pos=0;	//旋钮位置
	double dat_step=0;

	int8_t		*v_i8=NULL;
	uint8_t		*v_uint8_t=NULL;
	int16_t		*v_i16=NULL;
	int32_t		*v_i32=NULL;
	float 	*v_f16=NULL;
	double	*v_f32=NULL;
...
//--------【参数准备】
	switch(type)
	{
		case INT8:	v_i8=(int8_t *)value;break;
		case UINT8:	v_uint8_t=(uint8_t *)value;break;  
		case INT32: v_i32=(int32_t *)value; break;
		case FLT16: v_f16=(float *)value; break;
		case FLT32: v_f32=(double *)value; break;
		case INT16: ;	
		default:	v_i16=(int16_t *)value; break;
	}
//--------【计算显示行】
	if(True==_OptionsDisplay_Judge(USN))
	{		
...
//--------【键值解析】
			...
			if(bit_istrue(SOLGUI_CSR,bit(0)))			//如果处于SPIN占用中
			{
				...
				if(cur_key==SOLGUI_KEY_UP)														//数值加步进
				{
					switch(type)
					{
						case INT8:	{if(((*v_i8)+dat_step)<=max)(*v_i8)+=(int8_t)dat_step;}break;
						case UINT8:	{if(((*v_uint8_t)+dat_step)<=max)(*v_uint8_t)+=(uint8_t)dat_step;}break;  
						case INT32: {if(((*v_i32)+dat_step)<=max)(*v_i32)+=(int32_t)dat_step;}break;
						case FLT16: {if(((*v_f16)+dat_step)<=max)(*v_f16)+=(float)dat_step;}break;
						case FLT32: {if(((*v_f32)+dat_step)<=max)(*v_f32)+=(double)dat_step;}break;
						case INT16: ;	
						default:	{if(((*v_i16)+dat_step)<=max)(*v_i16)+=(int16_t)dat_step;}break;
					}	
				}
				else if(cur_key==SOLGUI_KEY_DOWN)												//数值减步进
				{
					switch(type)
					{
						case INT8:	{if(((*v_i8)-dat_step)>=min)(*v_i8)-=(int8_t)dat_step;}break;
						case UINT8:	{if(((*v_uint8_t)-dat_step)>=min)(*v_uint8_t)-=(uint8_t)dat_step;}break;  
						case INT32: {if(((*v_i32)-dat_step)>=min)(*v_i32)-=(int32_t)dat_step;}break;
						case FLT16: {if(((*v_f16)-dat_step)>=min)(*v_f16)-=(float)dat_step;}break;
						case FLT32: {if(((*v_f32)-dat_step)>=min)(*v_f32)-=(double)dat_step;}break;
						case INT16: ;	
						default:	{if(((*v_i16)-dat_step)>=min)(*v_i16)-=(int16_t)dat_step;}break;
					}	
				}
...
//--------【选项：变量名&数值绘制】
		switch(type)
		{
			...
			case UINT8:	SOLGUI_printf(68,y_disp,F6X8,"%d",*v_uint8_t);break; 
...
void SOLGUI_Widget_OptionText(uint8_t USN,const uint8_t* str,...)					//选项文本
{
	va_list ap;
	uint32_t y_disp=0;	//该选项应该显示的行位置
//	uint8_t t[SCREEN_X_PAGE-2];		//字符缓存
	if(bit_istrue(SOLGUI_CSR,bit(1))) return;	//全屏占用
//--------【参数过滤】
	if(USN>=OPTIONS_MAX) return;		//USN只允许取0~OPTIONS_MAX-1
//	_String_LenCtrlCpy(SCREEN_X_PAGE-2,t,(uint8_t *)str);
//--------【计算显示行】
	if(True==_OptionsDisplay_Judge(USN))
	{		
		y_disp=_OptionsDisplay_coorY(USN);
...
void _String_LenCtrlCpy(u16 dest_size,uint8_t *dest,uint8_t *sour)		  	//带字数限制的复制
{
	if(strlen((const char *)sour)>dest_size) strncpy((char *)dest,(char *)sour,dest_size);	 
	else strcpy((char *)dest,(char *)sour);	
}

void SOLGUI_Widget_Edit(uint8_t USN,const uint8_t *name,u16 char_num,uint8_t *buf)			//文本编辑器
{	
	uint32_t y_disp=0;	//该选项应该显示的行位置
	uint8_t cur_key=0;	//键值
	u16 i=0,j=0,buf_i=0;
	uint8_t edit_thumbnail_buf[EDIT_THUMBNAIL_SIZE];	//缩略内容
	static uint8_t edit_buf[EDIT_BUF_SIZE];			//编辑器缓存buf	
...
void SOLGUI_Widget_Text(uint32_t x0,uint32_t y0,uint8_t mode,const uint8_t* str,...)
{
	va_list ap;
//--------【当前状态】
	if(bit_istrue(SOLGUI_CSR,bit(1))) return;	//全屏占用
...
void SOLGUI_Widget_Spectrum(uint32_t x0,uint32_t y0,uint32_t xsize,uint32_t ysize,int32_t max,int32_t min,u16 val_num,int32_t value[])
{
	int32_t swap=0;
	u16 i=0;
	double delta_x=0,delta_y=0;
	double x_p=0;
	uint32_t x_last=x0,y_last=y0,y_zero=0;
	int32_t val_now=0;

	uint32_t x_now=0,y_now=0;
...
//------------【曲线绘制】
	for(i=0;i<xsize;i++)
	{
		...
		x_now=x0+i;
		if((val_now>=min)&&(val_now<max)) y_now=y0+(uint32_t)((val_now-min)*delta_y);
...
//------------【零点绘制】
	if(min<0)
	{
		y_zero=y0+(uint32_t)((-min)*delta_y);
...
void SOLGUI_Widget_Oscillogram(uint32_t x0,uint32_t y0,uint32_t xsize,uint32_t ysize,int32_t max,int32_t min,WaveMemBlk *wmb)	  //波，要通过探针输入数据更新
{
//--------【当前状态】
	if(bit_istrue(SOLGUI_CSR,bit(1))) return;	//全屏占用
//--------【绘图】
...
void SOLGUI_Oscillogram_Probe(WaveMemBlk *wmb,int32_t value)									//探针
{
	u16 f=0,b=1;
//--------【数据队列操作】				
...
void SOLGUI_Widget_Picture(uint32_t x0,uint32_t y0,uint32_t xsize,uint32_t ysize,const uint8_t *pic,uint32_t x_len,uint32_t y_len,uint8_t mode)	//带缩小适配的图片控件 
{
	float fw=0,fh=0;
	u16 temp_x=0,temp_y=0;
	u16 y_i=0,x_i=0;
	uint32_t y0_t=y0+ysize;	//将原点改为左下角（原本原点为右上角）
	uint8_t m1=0;
//--------【当前状态】
	if(bit_istrue(SOLGUI_CSR,bit(1))) return;	//全屏占用
