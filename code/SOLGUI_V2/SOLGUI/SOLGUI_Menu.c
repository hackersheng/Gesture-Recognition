...
uint8_t cur_key=0;			//全局键值

uint8_t SOLGUI_CSR=0;		//占用标志寄存器（若非零即表示占用，不能退出当前页面）
/*----------------【占用标志寄存器】------------------	
...
void FIFO_EnQueue(uint8_t KeyCode)
{
...
{
	uint8_t left_len=0,title_len=0;
...
void SOLGUI_InputKey(uint8_t key_value) 					//【非阻塞】从系统中获取当前按键键值存入键池
{
	FIFO_EnQueue(key_value);
}

uint8_t SOLGUI_GetCurrentKey(void) 						//【非阻塞】获取GUI当前的全局键值
{
  	return(cur_key); 	
}
