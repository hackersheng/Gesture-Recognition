...
FontInfo _fontInfo_6x8={0x01,6,8,(uint8_t*)_Font6x8};	   	//Font6x8_ASCII

#if FONT4X6_EN==1
FontInfo _fontInfo_4x6={0x02,4,6,(uint8_t*)_Font4x6};		//Font4x6_ASCII
#endif
#if FONT8X8_EN==1
FontInfo _fontInfo_8x8={0x04,8,8,(uint8_t*)_Font8x8};	   	//Font8x8_ASCII
#endif
#if FONT8X10_EN==1
FontInfo _fontInfo_8x10={0x08,8,10,(uint8_t*)_Font8x10};	   	//Font8x10_ASCII
#endif

//---------------------【字库选择器】
FontInfo SOLGUI_SwitchFont(uint8_t mode)
{
	switch(mode)
	{
		case F6X8:
...
//########################【内部使用】########################
//----------------【在屏幕上输出字符】
void SOLGUI_PutChar(uint32_t x,uint32_t y,uint8_t ch,uint8_t mode)//显示单个字符（mode：1正常显示，0高亮显示）
{
	uint8_t temp,m=1,tl,x0=x;
	int8_t t;
...
void SOLGUI_PutString(uint32_t x,uint32_t y,const uint8_t *str,uint8_t mode)	//显示字符串（mode：1正常显示，0高亮显示）
{
	uint8_t j=0;
	FontInfo fi=SOLGUI_SwitchFont(mode);
//-------------【字库选择】
...
{
	uint8_t xpp=x;
	uint8_t f,r,fl=0,l=3,lt;		//默认留3位小数,小数可留0~7位
	uint8_t i,j,w,lp;
	uint32_t v;
	int8_t c, d, s[16], *p;
	int16_t res, chc, cc,ll;
	int16_t kh,kl,pow=1;
	double k;
	FontInfo fi=SOLGUI_SwitchFont(mode);
...
//----------------【在屏幕上格式化输出字符串】
void SOLGUI_printf(uint32_t x,uint32_t y,uint8_t mode,const uint8_t* str,...)			//外部使用的API版printf函数
{
	va_list arp;
