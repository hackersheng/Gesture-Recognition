...
#define ClockFrequency   43400000 //时钟频率
...
uint8_t FDC2214_GetChannelData(FDC2214_channel_t channel, uint32_t *data)
{
    ...
    while(timeout > 0 && !(status & bitUnreadConv))//检查是否有未读数据
    {
...
/*!
 *  @brief      设置FDC2214一个通道的转换时间，转换时间(tCx)=(CHx_RCOUNTˣ16) / fREFx。
 *  @param      channel     通道
...
/*!
 *  @brief      设置FDC2214一个通道的建立时间，使LC传感器可以在通道x开始转换之前稳定下来，建立时间(tSx) = (CHx_SETTLECOUNTˣ16) ÷ fREFx。
                CHx_SETTLECOUNT = 0x00, 0x01时(tSx) = 32 ÷ fREFx。
 *  @param      channel     通道
 *  @param      count       CHx_SETTLECOUNT
 *  @since      v1.0
 *  @note       CHx_SETTLECOUNT > Vpk × fREFx × C × PI^2 / (32 × IDRIVEX)
...
float FDC2214_CalculateFrequency(FDC2214_channel_t channel, uint32_t datax)
{
    ...
    frequency = (float)(CHx_FIN_SEL[channel] * fREFx[channel] * datax) / 268435456.0;//2^28
...
uint8_t FDC2214_Init(void)
{
    ...
    if(deviceID == FDC2214_ID)
    {
        ...
        FDC2214_SetClockSource(0);//1 外部时钟源  0内部晶振
